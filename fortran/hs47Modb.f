!     ------------------------------------------------------------------
!     File hs47Modb.f :: solves a modified version of hs47.
...
!     ------------------------------------------------------------------
     ...
     &     Prob*8, Names(nName)*8
      integer
     &     indJ(maxne), hs(maxn+maxm)
      integer
     &     locJ(maxn+1)
      double precision
     &     Jcol(maxne), bl(maxn+maxm), bu(maxn+maxm),
...
!     ------------------------------------------------------------------
     ...
     &     n, ne, nInf, nlocJ, nnCon, nnJac, nnObj,
     &     nOut, nS
      double precision
     &     Obj, ObjAdd, sInf
      external
     &     hs47ModCon, hs47ModObj
!     ------------------------------------------------------------------
...
!+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
     ...
     &     none
      integer
     &     maxm, maxn, maxne, INFO, m, n, neJ, nlocJ,
     &     nnCon, nnObj, nnJac, iObj,
     &     indJ(maxne), hs(maxn+maxm), locJ(maxn+1)
      double precision
     &     ObjAdd, Jcol(maxne) , bl(maxn+maxm), bu(maxn+maxm),
     &     x(maxn+maxm), pi(maxm)
      character
     &     Prob*8

!     ------------------------------------------------------------------
!     hs47ModData generates data for problem hs47Modb.
...
!     ne     is the number of nonzeros in Jcol(*).
...
!     ------------------------------------------------------------------
      ...
      end ! subroutine hs47ModData

!+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++

      subroutine hs47ModObj
...
!     ==================================================================
     ...
     &          + (x(3)-x(5))**4 + (x(5)-x(4))**4
      ...
      end ! subroutine hs47ModObj

!+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++

      subroutine hs47ModCon
...
!     Print something on the first and last entry.
      ...
      if (      State .eq. 1) then ! First
         if (Out .gt. 0) write(Out, '(/a)') ' Starting  hs47Modb'
      else  if (State .ge. 2) then ! Last
         if (Out .gt. 0) write(Out, '(/a)') ' Finishing hs47Modb'
      end if
